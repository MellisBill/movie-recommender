...
<div class="flex flex-col mx-auto">
	...
	<div class="py-8">
		...
		<div class="flex flex-row flex-wrap space-x-2 max-w-lg">
			{#each addedGenres as addedGenre, index}
			<button
				class="flex flex-row btn-outline rounded-3xl border-2 px-2 py-1 my-2 active:border-yellow-500 hover:border-yellow-500 hover:bg-transparent"
				on:click={() => addedGenres = addedGenres.filter(genre => genre !== addedGenre)}
			>
				{addedGenre}
				<svg xmlns="http://www.w3.org/2000/svg" height="24px" width="24px" viewBox=" 0 0 43 43" stroke="white" fill="white"
					><path
	...
	<div class="flex flex-row flex-wrap max-w-lg justify-center space-x-4">
		{#each genres as genre}
			<button
				...
				on:click={() => addedGenres = [...addedGenres, genre.description]}
			>
				{genre.description}
			</button>
		{/each}
